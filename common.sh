...
# 5. Install Kubernetes Components (v1.29.0):
...
# 1.29.0 specific key URL
curl -fsSL https://pkgs.k8s.io/release/stable/v1.29.0/deb/Release.key | sudo gpg --dearmor -o /etc/apt/keyrings/kubernetes-apt-keyring.gpg
sleep 2

# Change the repository to version 1.29.0
echo 'deb [signed-by=/etc/apt/keyrings/kubernetes-apt-keyring.gpg] https://pkgs.k8s.io/release/stable/v1.29.0/deb/ /' | sudo tee /etc/apt/sources.list.d/kubernetes.list
...
# Prevent updates for the Kubernetes components
